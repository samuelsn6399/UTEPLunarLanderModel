...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(ColdGas_GNC_Model_PIL_M) == (NULL)) &&
    !rtmGetStopRequested(ColdGas_GNC_Model_PIL_M);
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    ColdGas_GNC_Model_PIL_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(ColdGas_GNC_Model_PIL_M) == (NULL)) &&
                      !rtmGetStopRequested(ColdGas_GNC_Model_PIL_M));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(ColdGas_GNC_Model_PIL_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  ColdGas_GNC_Model_PIL_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(ColdGas_GNC_Model_PIL_M, 0);

  // Initialize model
  ColdGas_GNC_Model_PIL_initialize();
